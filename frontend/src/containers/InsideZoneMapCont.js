...
class InsideZoneMapCont extends Component {
    ...
    render() {
        ...
        let toggled_markers = this.props.toggled_quests.map(q => {
                ...
                maxBoundsViscosity='1' scrollWheelZoom={true} style={{height: '825px', width: '100%'}} >
                    ...
                    {Object.entries(clustered_markers).map(([key, value]) => {
                        if (value.length > 1) {
                            ...
                            let toggled_quests = [];
                            value.map(v => {
                                if (this.props.toggled_quests.includes(v.quest)) {
                            ...
                            if (value[0].start_icon) {
                                let start = null;
                                    if (this.props.toggled_quests.includes(value[0].quest)) {
                                ...
                                return <LayerGroup>
                                    ...
                                    <Marker key={Math.random()} 
                                    position={this.props.revertLat(value[0].npc.npc_location_x, value[0].npc.npc_location_y)} 
                                    icon={icon} eventHandlers={{ click: () => this.props.toggleQuest(value[0], 
                                        active_in_zone_quests) }} >
                                        <Tooltip>
                                ...
                                return <LayerGroup>
                                    ...
                                    <Marker key={Math.random()} 
                                    position={this.props.revertLat(value[0].npc.npc_location_x, value[0].npc.npc_location_y)} 
                                    icon={step_icon} eventHandlers={{ click: () => this.props.toggleQuest(value[0], 
                                        active_in_zone_quests) }} >
                                        <Tooltip>
                                            <h6 className='text-center'>{value[0].quest.quest_name}</h6>
                                            <h6 className='text-center'>{value[0].npc.npc_name}</h6>
                    ...
                    <Accordion.Item eventKey='0'>
                        ...
                        <Accordion.Body>
                            {active_in_zone_quests.map(quest => {
                                let theme = '';
                                if (this.props.toggled_quests.length > 0 && this.props.toggled_quests.includes(quest[0])) {
                                    theme = 'btn-primary';
                                } else {
                                    theme = 'btn-secondary';
                                };
...
export default connect(mapStateToProps)(InsideZoneMapCont);